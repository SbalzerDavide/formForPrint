...
<template>
  <div class="print">
    <button class="confirm hide-print" @click="print">
      Conferma
    </button>
    <button class="come-back-to-form hide-print" @click="comeBack">
      Modifica dati
    </button>
    <header>
      <div class="top-header d-flex justify-between">
        <div class="img">
          ...
          <!-- <img 
            ...
            > -->
          <img v-else-if="office === 'Pralboino'" class="img-pralboino" src="@/img/headerPralboino.jpg">
    ...
    <main>
      ...
      <section class="medical-history">
        ...
        Età materna: {{ age }} anni, Altezza {{ height }}cm, Peso normale {{ normalWeight }}kg, Peso attuale {{
          actualWeight }}kg, (BMI {{ bmi }})
      </section>
      <section class="pregnancy-date">
        <div v-show="pregnancy?.start" class="row active-pregnancy">Data ultime mestruazioni: {{
          formattingPregnancyStart }}</div>
        <div class="row date">
        ...
        <div v-if="pregnancy?.epocaGestazionale" class="row" :class="enableCRLReDate ? '' : 'active-pregnancy'">
          Epoca gestazionale
        ...
        <div v-if="enableCRLReDate && pregnancy.reDateFromCrl" class="row"
      ...
      <section v-if="showAnatomia" class="anatomy">
        ...
        <div class="anatomy-container">
          ...
          <div class="anatomy-footer">
            Per “visualizzato” si intende che l'organo o apparato indagato presenta caratteristiche ecografiche regolari
      ...
      <section v-if="showDoppler" class="doppler">
        ...
        <div class="doppler-item" v-for="(item, index) in doppler" :key="index">
          ...
          <div v-if="item.value" class="more-doppler">
            <div v-if="(item.name == 'PIUDX' || item.name == 'PIUSX') && item.incisura" class="incisura">notch presente
            </div>
            <div v-else-if="(item.name == 'PIUDX' || item.name == 'PIUSX') && !item.incisura" class="incisura">notch
              assente</div>
      ...
      <section class="informativa">
        <p>La Persona assistita ha avuto modo di ricevere una adeguata informazione su scopi e limiti dell’esame
          ...
          possibilità di rivolgere eventuali quesiti), e attraverso la lettura di un foglio informativo allegato al
          presente referto. Su esplicita richiesta del Medico Ecografista la persona assistita riferisce di aver
          compreso
          tutte informazioni ricevute.
          Si informa la persona assistita /Coppia degli esiti dell'esame ecografico effettuato e dei suoi limiti nella
          individuazione delle anomalie fetali.
        </p>
