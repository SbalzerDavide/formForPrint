...
<template>
  <div v-if="!showPrint" class="form">
    <PopupMessage :content="popupMessage" position="bottom" :type="popupType" :show="triggerPopup"
    ...
    <section class="patient">
      ...
      <div class="more-info">
        ...
        <textarea v-else v-model="patientMoreText" placeholder="Aggiungi ulteriori informazioni" rows="4">
        </textarea>
      </div>
    ...
    <div class="double">
      <section class="pregnancy">
        ...
        <div class="switch">
          <div @click="activeDateSelection = 'start'" class="selStart"
            :class="activeDateSelection === 'start' ? 'act' : ''">
            Ultima mestruazione
          </div>
          <div @click="activeDateSelection = 'end'" :class="activeDateSelection === 'end' ? 'act' : ''" class="selEnd">
            Data prevista parto
          </div>
          <div class="active" :class="activeDateSelection"></div>
        </div>
        <input v-if="activeDateSelection == 'start'" @change="calcPregnancyDate" v-model="startDate" type="date">
        <input v-else-if="activeDateSelection == 'end'" @change="calcPregnancyDate" v-model="deliveryDate" type="date">
        <div class="calc-date">
        ...
        <div class="more-info">
          ...
          <textarea v-else v-model="pregnancyMoreText" placeholder="Aggiungi ulteriori informazioni" rows="4">
          </textarea>
        </div>
      ...
      <section class="eco">
        ...
        <div class="tool">
          ...
          <input type="text" v-if="ecoTool == 'altro'" v-model="otherTool" placeholder="Specificare nome strumento"
            class="other-tool">
        </div>
        <div class="type">
        ...
        <div class="more-info">
          ...
          <textarea v-else v-model="ecoMoreText" placeholder="Aggiungi ulteriori informazioni" rows="4">
          </textarea>
        </div>
      </section>
    </div>
    <section class="biometria-fetale">
      ...
      <div class="biometria-item" v-for="(item, index) in biometriaFetale" v-show="checkShowBiometria(item)"
        :key="index">
      ...
      <div class="more-info">
        ...
        <textarea v-else v-model="biometriaMoreText" placeholder="Aggiungi ulteriori informazioni" rows="4">
        </textarea>
      </div>
    </section>
    <section v-show="redatingPanel" class="re-dating-panel">
      <div class="re-dating-background" @click="redatingPanel = false">
      </div>
      <div class="re-dating-container">
        ...
        <div class="old-date">
          <span>Data originale: </span> {{ parseInt(decimalWeeks) }} settimane + {{ ((decimalWeeks -
            parseInt(decimalWeeks)) * 7).toFixed(0) }} giorni
        </div>
        <div class="new-date">
          ...
          <div class="re-dating-input">
            <div class="number">
              <div>
                <input type="number" v-model="crlWeeks" @change="setManuallyCrlReDate('weeks')"> Settimane +
              </div>
              <div>
                <input type="number" v-model="crlDays" @change="setManuallyCrlReDate('weeks')"> Giorni
              </div>
            </div>
            <div class="delivery">
              <label>Termine eco:</label>
              <input type="date" v-model="deliveryDateFromCRL" @change="setManuallyCrlReDate('date')">
            </div>
          </div>
        </div>
        <div class="action">
    ...
    <section class="anatomy">
      ...
      <div class="anatomy-container">
        <div v-for="(element, index) in anatomy" :key="index" v-show="element.ecoType.includes(ecoType.value)"
          ...
          <div class="left">
            ...
            <label :for="element.value" :class="element.checked ? 'anatomy-checked' : ''">{{ element.name }}</label>
          </div>
          <input type="checkbox" :name="element.value" :id="element.value" v-model="element.checked">
    ...
    <section class="doppler">
      ...
      <div class="doppler-item" v-for="(item, index) in doppler"
    ...
    <section class="more">
      ...
      <div class="more-info">
        ...
        <textarea v-else v-model="lastMoreText" placeholder="Aggiungi ulteriori informazioni" rows="4">
        </textarea>
      </div>
  ...
  <Print v-else :user="user" @comeBack="comeBack" :office="office" :patient="name + ' ' + surname"
    ...
    :direction="ecoType.value !== '1T' ? direction : ''" :liquid="liquid" :placenta="placenta"
    :patientMore="patientMoreText" :pregnancyMore="pregnancyMoreText" :ecoMore="ecoMoreText"
    :biometriaMore="biometriaMoreText" :lastMore=lastMoreText :conclusion="conclusion" />
</template>
